// From ffmpeg-next library example code
// For some reason, mine doesn't work (I have not found the problem), so i am using the official code
...
pub fn video(input_file: &str, output_file: &str) 
{
    ...
    format::context::input::dump(&input, 0, Some(&input_file));

    let best_video_stream_index = input.streams().best(media::Type::Video).map(|stream| stream.index());
    let mut stream_mapping: Vec<isize> = vec![0; input.nb_streams() as _];
    let mut ist_time_bases = vec![Rational(0, 0); input.nb_streams() as _];
    let mut ost_time_bases = vec![Rational(0, 0); input.nb_streams() as _];
    ...
    format::context::output::dump(&output, 0, Some(&output_file));
    output.write_header().unwrap();

    for (ost_index, _) in output.streams().enumerate() 
    {
        ost_time_bases[ost_index] = output.stream(ost_index as _).unwrap().time_base();
    }
